...
export function ThinkingChip({ text, title, isActive, onClick }: ThinkingChipProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border cursor-pointer transition-all duration-150 text-[11px] max-w-[320px] ${
        isActive
    ...
    >
      ...
      <span className={`truncate ${isActive ? 'text-violet-400 thinking-indicator' : 'text-gray-400'}`}>{label}</span>
      {text.length > 0 && (
        <span className={`shrink-0 ${isActive ? 'text-violet-300/70' : 'text-gray-500'}`}>{text.length.toLocaleString()}</span>
      )}
      {isPending && (
