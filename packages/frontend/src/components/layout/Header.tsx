...
interface HeaderProps {
  ...
  onSelectSession?: (session: import('../../stores/session-store').SessionMeta) => void;
  username?: string;
  userRole?: string;
  sidebarOpen?: boolean;
}
...
function NavTabs({ onRequestFileTree }: { onRequestFileTree?: () => void }) {
  ...
  const activeKey = activeView === 'kanban' ? 'task'
    : activeView === 'schedules' ? 'task'
    : activeView === 'automations' ? 'task'
...
export function Header({
  ...
  onRequestFileTree, onSelectSession, username, userRole,
}: HeaderProps) {
  const { t } = useTranslation('layout');
  ...
  return (
    <header className="h-12 bg-surface-900/80 backdrop-blur-md border-b border-surface-800 flex items-center px-3 md:px-4 gap-2 md:gap-3 shrink-0 z-50">
      ...
      <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
        ...
        <NotificationBell onSelectSession={onSelectSession} />
