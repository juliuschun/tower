...
function buildCoreSystemPrompt(): string {
  return [
    ...
    '',
    '**```svg** — Use for polished static or animated SVG infographics (hero images, concept visualizations, branded diagrams). Raw SVG only (starts with `<svg`). Rendered inline — lighter than html-sandbox.',
    'Security: sanitized via DOMPurify (SVG + svgFilters profile). `<script>`, `<foreignObject>`, and `on*` handlers are stripped.',
    'Choose by context: complex concept explanation → animated; writing docs/README → static; structural diagrams (6+ nodes, ERD, sequence) → use mermaid instead.',
    '',
    '**```html-sandbox** — Use for interactive demos, custom UI prototypes, or complex visualizations not covered above. Heavier than ```svg (iframe overhead) — prefer ```svg when content is just SVG.',
    '',
    ...
    '',
    '**```diff** — Use for code before/after comparison. JSON: { "before": "...", "after": "...", "mode": "split" }',
    '',
